...
class Slitherlink {
public:
    Slitherlink(const vector_2d &grid) :
        nr(grid.size()),
        nc(grid[0].size()),
    ...
    bool solve(void) {
        vector_2d region(this->nr + 2, std::vector<int>(this->nc + 2, UNDET));
    ...
    void print_region(const vector_2d &region) {
        for (int i = 1; i < this->nr + 1; ++i) {
            for (int j = 1; j < this->nc + 1; ++j) {
                std::cout << region[i][j] << " ";
            }
    ...
    vector_2d region_solved;

private:
    bool solve_helper(const vector_2d &region) {
        vector_2d new_region = region;
    ...
    bool apply_heuristics(vector_2d &region) {
    ...
    bool is_available_partial_solution(vector_2d &region) {
    ...
    bool is_answer(vector_2d &region) {
    ...
    std::pair<int, int> find_region(vector_2d &region, int val) {
        FOR_CELL {
            if (region[i][j] == val) {
...
int main(void) {
    ...
    Slitherlink slink(grid);

    if (slink.solve()) {
        slink.print_solution_pretty();
    } else {
